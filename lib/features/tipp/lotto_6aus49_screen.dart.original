// Lotto 6aus49 Screen - Teil 1: Imports und State Class
...
class _Lotto6Aus49ScreenState extends State<Lotto6Aus49Screen> {
  ...
  // Superzahl Animation verbessert - bleibt auf finaler Zahl stehen
  Future<void> _animateSuperzahl() async {
    ...
    // Finale Superzahl setzen und Animation beenden
    if (mounted) {
      setState(() {
  ...
  // Hauptzahlen Animation 2x beschleunigt
  Future<void> _animateMainNumbers() async {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      await Future.delayed(Duration(milliseconds: 50)); // Statt 100ms
    }

    // Finale Zahlen setzen
    if (mounted) {
      setState(() {
        generatedNumbers = finalNumbers;
        currentAnimationNumbers = finalNumbers;
        animationCompleted = true;
        canInteract = true; // Interaktion wieder erlauben
      });
    }
  }
  ...
  void _generateNumbers() async {
    ...
    setState(() {
      ...
      canInteract = false; // Während Animation keine Interaktion
  ...
  Widget _buildSuperzahlSection(Orientation orientation) {
    return GestureDetector(
      ...
      child: Container(
        height: orientation == Orientation.portrait ? 60 : 40, // Um 1/3 verkleinert
        decoration: BoxDecoration(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              ...
              style: TextStyle(
                fontSize: orientation == Orientation.portrait ? 28 : 22, // Vergrößert
  ...
  Widget _buildNumbersGrid(Orientation orientation) {
    int crossAxisCount = orientation == Orientation.portrait ? 7 : 10; // Quermodus: 10 statt 7 Spalten
    double aspectRatio = orientation == Orientation.portrait ? 1.0 : 0.8; // Angepasstes Seitenverhältnis
  ...
  Widget _buildGeneratedNumbersSection() {
    if (!animationCompleted) return SizedBox.shrink(); // Erst nach Animation anzeigen
      ...
      body: OrientationBuilder(
        builder: (context, orientation) {
          return Container(
            ...
            child: Column(
              children: [
                // Superzahl Section - Höhe reduziert, Tipps rutschen hoch
                _buildSuperzahlSection(orientation),
                SizedBox(height: 16), // Reduzierter Abstand
                // Main Numbers Grid - Optimierte Flächennutzung
                Expanded(
                  child: _buildNumbersGrid(orientation),
                ),
                // Generated Numbers Display - Erst nach Animation
